 ...
 * stfb (ZTURN-S bwd terminator: drev comb DEINT in, IDFT + POST conj-w^1, DIRECT record stores at section taps — no TR4).
...
/* ===================== PROVENANCE =====================
 * Generated by: gen_radix.exe 8 --zp-stfb --zp-r0 8 --isa avx2 --uarch raptor_lake_avx2 --emit-c
 * Env overrides: (none)
 * Family: zsplit-pipeline
 * kind=stf radix=8 dir=bwd isa=avx2 r0=8 (ZTURN-S sectioned geometry)
    ...
    size_t Ls, size_t Gs, size_t OLs, size_t OGs, size_t count)
{
    ...
    for (size_t k = 0; k + 4 <= count; k += 4) {
        /* Z load edge (DEINT) */
        const __m256d _zl_0 = _mm256_loadu_pd(&zin[2*2*(size_t)k]);
        const __m256d _zh_0 = _mm256_loadu_pd(&zin[2*2*(size_t)k + 4]);
        ...
        const __m256d _zl_1 = _mm256_loadu_pd(&zin[2*((size_t)1*OLs + 2*(size_t)k)]);
        const __m256d _zh_1 = _mm256_loadu_pd(&zin[2*((size_t)1*OLs + 2*(size_t)k) + 4]);
        ...
        const __m256d _zl_2 = _mm256_loadu_pd(&zin[2*((size_t)2*OLs + 2*(size_t)k)]);
        const __m256d _zh_2 = _mm256_loadu_pd(&zin[2*((size_t)2*OLs + 2*(size_t)k) + 4]);
        ...
        const __m256d _zl_3 = _mm256_loadu_pd(&zin[2*((size_t)3*OLs + 2*(size_t)k)]);
        const __m256d _zh_3 = _mm256_loadu_pd(&zin[2*((size_t)3*OLs + 2*(size_t)k) + 4]);
        ...
        const __m256d _zl_4 = _mm256_loadu_pd(&zin[2*((size_t)4*OLs + 2*(size_t)k)]);
        const __m256d _zh_4 = _mm256_loadu_pd(&zin[2*((size_t)4*OLs + 2*(size_t)k) + 4]);
        ...
        const __m256d _zl_5 = _mm256_loadu_pd(&zin[2*((size_t)5*OLs + 2*(size_t)k)]);
        const __m256d _zh_5 = _mm256_loadu_pd(&zin[2*((size_t)5*OLs + 2*(size_t)k) + 4]);
        ...
        const __m256d _zl_6 = _mm256_loadu_pd(&zin[2*((size_t)6*OLs + 2*(size_t)k)]);
        const __m256d _zh_6 = _mm256_loadu_pd(&zin[2*((size_t)6*OLs + 2*(size_t)k) + 4]);
        ...
        const __m256d _zl_7 = _mm256_loadu_pd(&zin[2*((size_t)7*OLs + 2*(size_t)k)]);
        const __m256d _zh_7 = _mm256_loadu_pd(&zin[2*((size_t)7*OLs + 2*(size_t)k) + 4]);
        ...
        _mm256_storeu_pd(&zout[4*(size_t)k + 0], t30);
        _mm256_storeu_pd(&zout[4*(size_t)k + 4], t31);
        _mm256_storeu_pd(&zout[4*((size_t)2*OLs/2 + k) + 0], t132);
        _mm256_storeu_pd(&zout[4*((size_t)2*OLs/2 + k) + 4], t133);
        _mm256_storeu_pd(&zout[4*((size_t)1*OLs/2 + k) + 0], t79);
        _mm256_storeu_pd(&zout[4*((size_t)1*OLs/2 + k) + 4], t80);
        _mm256_storeu_pd(&zout[4*((size_t)3*OLs/2 + k) + 0], t136);
        _mm256_storeu_pd(&zout[4*((size_t)3*OLs/2 + k) + 4], t137);
        _mm256_storeu_pd(&zout[4*(size_t)k + 8], t105);
        _mm256_storeu_pd(&zout[4*(size_t)k + 12], t106);
        _mm256_storeu_pd(&zout[4*((size_t)2*OLs/2 + k) + 8], t140);
        _mm256_storeu_pd(&zout[4*((size_t)2*OLs/2 + k) + 12], t141);
        _mm256_storeu_pd(&zout[4*((size_t)1*OLs/2 + k) + 8], t120);
        _mm256_storeu_pd(&zout[4*((size_t)1*OLs/2 + k) + 12], t121);
        _mm256_storeu_pd(&zout[4*((size_t)3*OLs/2 + k) + 8], t144);
        _mm256_storeu_pd(&zout[4*((size_t)3*OLs/2 + k) + 12], t145);
    }
}
