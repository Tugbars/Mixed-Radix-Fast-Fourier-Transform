...
/* ===================== PROVENANCE =====================
 * Generated by: gen_radix.exe 8 --zp-stfbn --zp-r0 4 --isa avx2 --uarch raptor_lake_avx2 --emit-c
 * Env overrides: (none)
 * Family: zsplit-pipeline
 * kind=stfn radix=8 dir=bwd isa=avx2 r0=4 (ZTURN-S sectioned geometry)
...
void radix8_z_stfn_r4_bwd_avx2(
    ...
    size_t Ls, size_t Gs, size_t OLs, size_t OGs, size_t count)
{
    ...
    for (size_t k = 0; k + 4 <= count; k += 4) {
        ...
        _mm256_storeu_pd(&zout[4*((size_t)2*OLs + k) + 0], t132);
        _mm256_storeu_pd(&zout[4*((size_t)2*OLs + k) + 4], t133);
        _mm256_storeu_pd(&zout[4*((size_t)1*OLs + k) + 0], t79);
        _mm256_storeu_pd(&zout[4*((size_t)1*OLs + k) + 4], t80);
        _mm256_storeu_pd(&zout[4*((size_t)3*OLs + k) + 0], t136);
        _mm256_storeu_pd(&zout[4*((size_t)3*OLs + k) + 4], t137);
        _mm256_storeu_pd(&zout[4*(size_t)k + 8], t105);
        _mm256_storeu_pd(&zout[4*(size_t)k + 12], t106);
        _mm256_storeu_pd(&zout[4*((size_t)2*OLs + k) + 8], t140);
        _mm256_storeu_pd(&zout[4*((size_t)2*OLs + k) + 12], t141);
        _mm256_storeu_pd(&zout[4*((size_t)1*OLs + k) + 8], t120);
        _mm256_storeu_pd(&zout[4*((size_t)1*OLs + k) + 12], t121);
        _mm256_storeu_pd(&zout[4*((size_t)3*OLs + k) + 8], t144);
        _mm256_storeu_pd(&zout[4*((size_t)3*OLs + k) + 12], t145);
    }
}
