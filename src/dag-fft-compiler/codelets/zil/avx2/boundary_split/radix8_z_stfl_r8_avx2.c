 ...
 * tw_re = LOADED per-column stream at tw_re + 14*k: per 4-column group (R-1) records [c(k..k+3)][s(k..k+3)] of w^1..w^7 in leg order. tw_im unused. */
/* ===================== PROVENANCE =====================
 * Generated by: gen_radix.exe 8 --zp-stfl --zp-r0 8 --isa avx2 --uarch raptor_lake_avx2 --emit-c
 * Env overrides: (none)
 * Family: zsplit-pipeline
 * kind=stfl radix=8 dir=fwd isa=avx2 r0=8 (ZTURN-S sectioned geometry)
...
void radix8_z_stfl_r8_fwd_avx2(
    ...
    size_t Ls, size_t Gs, size_t OLs, size_t OGs, size_t count)
{
    ...
    for (size_t k = 0; k + 4 <= count; k += 4) {
        ...
        const __m256d lane_re_1 = _mm256_loadu_pd(&zin[4*((size_t)2*OLs/2 + k) + 0]);
        const __m256d lane_im_1 = _mm256_loadu_pd(&zin[4*((size_t)2*OLs/2 + k) + 4]);
        const __m256d lane_re_2 = _mm256_loadu_pd(&zin[4*((size_t)1*OLs/2 + k) + 0]);
        const __m256d lane_im_2 = _mm256_loadu_pd(&zin[4*((size_t)1*OLs/2 + k) + 4]);
        const __m256d lane_re_3 = _mm256_loadu_pd(&zin[4*((size_t)3*OLs/2 + k) + 0]);
        const __m256d lane_im_3 = _mm256_loadu_pd(&zin[4*((size_t)3*OLs/2 + k) + 4]);
        const __m256d lane_re_4 = _mm256_loadu_pd(&zin[4*(size_t)k + 8]);
        const __m256d lane_im_4 = _mm256_loadu_pd(&zin[4*(size_t)k + 12]);
        const __m256d lane_re_5 = _mm256_loadu_pd(&zin[4*((size_t)2*OLs/2 + k) + 8]);
        const __m256d lane_im_5 = _mm256_loadu_pd(&zin[4*((size_t)2*OLs/2 + k) + 12]);
        const __m256d lane_re_6 = _mm256_loadu_pd(&zin[4*((size_t)1*OLs/2 + k) + 8]);
        const __m256d lane_im_6 = _mm256_loadu_pd(&zin[4*((size_t)1*OLs/2 + k) + 12]);
        const __m256d lane_re_7 = _mm256_loadu_pd(&zin[4*((size_t)3*OLs/2 + k) + 8]);
        const __m256d lane_im_7 = _mm256_loadu_pd(&zin[4*((size_t)3*OLs/2 + k) + 12]);
