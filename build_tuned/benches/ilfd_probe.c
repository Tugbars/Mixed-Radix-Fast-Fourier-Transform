/* THE FLAT DIT PROBE (2026-09-04/05): the odd-N flat mixed-radix DIT engine
 * (src/core/oop/il_flatdit.h) standalone. Forward correctness against a
 * naive DFT at NATURAL indices + DC, the inverse as a roundtrip, then the
 * timing vs MKL. With --race every chain the 2D enumerator yields is built,
 * its per-stage FORMS raced (t2cp | msz; t2csg | t2csgn | t2csgn+order),
 * and the fastest plan is timed against MKL in the same run. --stages
 * prints the seed plan's per-stage times. Build:
 *   python build_tuned/build.py --compile --src build_tuned/benches/ilfd_probe.c --mkl --vfft */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
/* the plan's chain with the raced FORM on every stage: m = msz, n = t2csgn, o = t2csgn in natural-base order */
...
static void stage_times(const vfft_ilfd_plan_t *p, const double *zin, double *zout) {
    ...
    for (int s = 1; s < p->K; s++) printf(" | R%d cnt%zu x%zu%s: %.0f", p->R[s], p->D[s], p->nblk[s], p->msz[s] ? " msz" : p->tail[s] ? (p->gl[s] ? " t2csgn" : " t2csg") : " t2cp", ts[s]);
    printf("%s", "\n");
}
int main(int argc, char **argv) {
    ...
    printf("%-7s %-22s | dft@nat  dc      roundtrip | %9s %9s %6s | %9s %9s %6s\n", "N", "flat chain (m,n,o)", "fwd(ns)", "mkl(ns)", "f/mkl", "bwd(ns)", "mkl(ns)", "f/mkl");
    for (int i = 0; i < n; i++) {
        const int N = NS[i];
        ...
        double dc = fabs(z[0]-s0r) + fabs(z[1]-s0i), wn = 0, rt = 0;
        for (int t = 0; t < 6; t++) {
        ...
        if (p->bwd_ok) { vfft_ilfd_execute_bwd(p, z, y); for (int j = 0; j < 2*N; j++) { double d = fabs(y[j]/N - x[j]); if (d > rt) rt = d; } }
        const int ok = (wn < 1e-9 * sqrt((double)N) && dc < 1e-9 * N && p->bwd_ok && rt < 1e-9 * sqrt((double)N));
        if (!ok) bad++;
        ...
        {   vfft_ilfd_plan_t *best = p; int owned = 0;
            ...
            /* final same-run timing: flat best vs MKL, both directions, alternated */
            double tm = 1e300, tb = 1e300, tmb = 1e300; tf = 1e300;
            ...
            if (DftiCreateDescriptor(&hm, DFTI_DOUBLE, DFTI_COMPLEX, 1, (MKL_LONG)N) == 0 &&
                ...
                for (int r = 0; r < 9; r++) {
                    ...
                    if (best->bwd_ok) { t0 = now_ns(); vfft_ilfd_execute_bwd(best, z, y); t0 = now_ns()-t0; if (t0 < tb) tb = t0; }
                    t0 = now_ns(); DftiComputeBackward(hm, z, y); t0 = now_ns()-t0; if (t0 < tmb) tmb = t0;
                }
                DftiFreeDescriptor(&hm);
            }
            printf("%-7d %-22s | %.1e %.1e %.1e %s | %9.0f %9.0f %5.2fx | %9.0f %9.0f %5.2fx\n",
                   N, cs, wn, dc, rt, ok ? "OK " : "BAD", tf, tm, tm/tf, tb, tmb, tmb/tb);
            fflush(stdout);
