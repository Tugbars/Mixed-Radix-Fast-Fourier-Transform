 ...
 * stfnl (NATURAL-ORDER ZTURN-S terminator: section taps at kn = 4*rhoinv[k/4] via the tw_im-carried table, LOADED twiddle stream in LOOP order (the plan builds the natural table in rho order), REINT stores contiguous ascending = natural interleaved out), fwd.
 * CONTRACT: count % 4 == 0 (4 columns per iteration).
 * tw_re = LOADED per-column stream at tw_re + 14*k: per 4-column group (R-1) records [c(k..k+3)][s(k..k+3)] of w^1..w^7 in leg order. tw_im carries the rho table; the stream is in LOOP order (record k/4 = plane column kn's twiddles). */
/* ===================== PROVENANCE =====================
 * Generated by: gen_radix.exe 8 --zp-stfnl --zp-r0 8 --isa avx2 --uarch raptor_lake_avx2 --emit-c
 * Env overrides: (none)
 * Family: zsplit-pipeline
 * kind=stfnl radix=8 dir=fwd isa=avx2 r0=8 (ZTURN-S sectioned geometry)
...
void radix8_z_stfnl_r8_fwd_avx2(
    ...
    size_t Ls, size_t Gs, size_t OLs, size_t OGs, size_t count)
{
    ...
    for (size_t k = 0; k + 4 <= count; k += 4) {
        ...
        const double *zn = zin + 4*kn;
        /* ZTURN-S section-tap load edge (2 records/section, 128 B contiguous, no shuffles) */
        const __m256d lane_re_0 = _mm256_loadu_pd(&zn[0]);
        const __m256d lane_im_0 = _mm256_loadu_pd(&zn[4]);
        const __m256d lane_re_1 = _mm256_loadu_pd(&zn[4*((size_t)2*OLs/2) + 0]);
        const __m256d lane_im_1 = _mm256_loadu_pd(&zn[4*((size_t)2*OLs/2) + 4]);
        const __m256d lane_re_2 = _mm256_loadu_pd(&zn[4*((size_t)1*OLs/2) + 0]);
        const __m256d lane_im_2 = _mm256_loadu_pd(&zn[4*((size_t)1*OLs/2) + 4]);
        const __m256d lane_re_3 = _mm256_loadu_pd(&zn[4*((size_t)3*OLs/2) + 0]);
        const __m256d lane_im_3 = _mm256_loadu_pd(&zn[4*((size_t)3*OLs/2) + 4]);
        const __m256d lane_re_4 = _mm256_loadu_pd(&zn[8]);
        const __m256d lane_im_4 = _mm256_loadu_pd(&zn[12]);
        const __m256d lane_re_5 = _mm256_loadu_pd(&zn[4*((size_t)2*OLs/2) + 8]);
        const __m256d lane_im_5 = _mm256_loadu_pd(&zn[4*((size_t)2*OLs/2) + 12]);
        const __m256d lane_re_6 = _mm256_loadu_pd(&zn[4*((size_t)1*OLs/2) + 8]);
        const __m256d lane_im_6 = _mm256_loadu_pd(&zn[4*((size_t)1*OLs/2) + 12]);
        const __m256d lane_re_7 = _mm256_loadu_pd(&zn[4*((size_t)3*OLs/2) + 8]);
        const __m256d lane_im_7 = _mm256_loadu_pd(&zn[4*((size_t)3*OLs/2) + 12]);
        ...
        _mm256_storeu_pd(&zout[2*((size_t)4*OLs + 2*(size_t)k)], _mm256_unpacklo_pd(_pr_4, _qi_4));
        _mm256_storeu_pd(&zout[2*((size_t)4*OLs + 2*(size_t)k) + 4], _mm256_unpackhi_pd(_pr_4, _qi_4));
        const __m256d _pr_5 = _mm256_permute4x64_pd(t133, 0xD8);
        const __m256d _qi_5 = _mm256_permute4x64_pd(t134, 0xD8);
        _mm256_storeu_pd(&zout[2*((size_t)5*OLs + 2*(size_t)k)], _mm256_unpacklo_pd(_pr_5, _qi_5));
        _mm256_storeu_pd(&zout[2*((size_t)5*OLs + 2*(size_t)k) + 4], _mm256_unpackhi_pd(_pr_5, _qi_5));
        const __m256d _pr_6 = _mm256_permute4x64_pd(t99, 0xD8);
        const __m256d _qi_6 = _mm256_permute4x64_pd(t100, 0xD8);
        _mm256_storeu_pd(&zout[2*((size_t)6*OLs + 2*(size_t)k)], _mm256_unpacklo_pd(_pr_6, _qi_6));
        _mm256_storeu_pd(&zout[2*((size_t)6*OLs + 2*(size_t)k) + 4], _mm256_unpackhi_pd(_pr_6, _qi_6));
        const __m256d _pr_7 = _mm256_permute4x64_pd(t131, 0xD8);
        const __m256d _qi_7 = _mm256_permute4x64_pd(t132, 0xD8);
        _mm256_storeu_pd(&zout[2*((size_t)7*OLs + 2*(size_t)k)], _mm256_unpacklo_pd(_pr_7, _qi_7));
        _mm256_storeu_pd(&zout[2*((size_t)7*OLs + 2*(size_t)k) + 4], _mm256_unpackhi_pd(_pr_7, _qi_7));
    }
}
